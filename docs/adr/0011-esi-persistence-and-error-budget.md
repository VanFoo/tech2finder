...
Request concurrency is governed by the **error budget, not a fixed number**: N requests in flight are N simultaneous potential errors, and being locked out is unacceptable. `X-ESI-Error-Limit-Remain` and `-Reset` are read on every response, in-flight requests are capped at `min(configured concurrency, remaining − safety floor)`, and the scan halts and waits for the reset window when the budget runs low. The **safety floor defaults to 50** — half of ESI's 100-error window held in reserve.

That floor produces a throttle-down band rather than a cliff: the cap stays at 15 all the way down to 65 remaining, then degrades over the next 15 errors before halting at 50. Nothing is bought by spending the reserve — the bulk scan is a one-off cost and every later scan is incremental — while errors during a bulk scan tend to arrive in bursts from a degradation rather than one at a time, so the headroom is what lets the scan notice and stop cleanly instead of discovering the problem at zero. There is a **hard ceiling of 15 concurrent requests**, which the budget-derived cap may only lower, never raise.
